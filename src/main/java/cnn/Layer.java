...
class Layer {
	...
	protected void init_indexes_GPU_Kernels() {
    	switch (type) {
     	case Config.CONVOLUTION:
    		if (Config.FLAT_ARRAYS) kernel_linear.put(weights.getDataFlat());
    		else kernel_linear.put(weights.getData3D());
    		break;
     	case Config.RELU:  
    	case Config.SIGMOID:  
    	case Config.TANH:
    		if (Config.FLAT_ARRAYS) kernel_perceptron.put(this.indexes.getDataFlat());
    		else kernel_perceptron.put(weights.getData3D());
    		break;
    	case Config.POOLMAX:  
    	case Config.POOLAVG:
    		if (Config.FLAT_ARRAYS) kernel_pool.put(this.indexes.getDataFlat());
    		else kernel_pool.put(weights.getData3D());
    		break;
    	case Config.SOFTMAX:  
    	}
    	return;
	...
	/**
	 * End of section with easily factored methods.
	 * If polymorphism used, next methods could be specialized
	 */
	...
	public Layer(
				...
				for (int output_layer = 0; output_layer < D_o; output_layer ++) {
					...
					p[output_layer][Field.getIndex(x, X_o, y, Y_o)] =
						new Perceptron (filter[output_layer], D_i, xi, X_i, yi, Y_i, dilation, type, output_layer,
								perceptron_indexes, (output_layer == crt_index_layer) );
				}
				xi += stride;
			}
			yi += stride;
		...
		if (type == Config.SOFTMAX) softmax_aggregates = new float[output.length_1()];
	...
	void init_SoftMaxKernelFlat() {
		...
		final int INPUT_LENGTH = this.input.length_1();
		final int INPUT_0_LENGTH = this.input.length_2();
	
		if (false) { 
		kernel_softmax = new Kernel() {
			private static float getV(float[] data, int x1, int dim1, int x2, int dim2) {
				return data[Field.getIndex(x1, dim1, x2, dim2)];
			}
			private static void setV(float[] data, int x1, int dim1, int x2, int dim2, float val) {
	...
	void init_SoftMaxKernelFlat2() {
		...
		kernel_softmax = new Kernel() {
	   	    private static int getIndex(int x, int X, int y, int Y) {
	   	    	return X*y + x;
	   	     }
			private static float getV(float[] data, int x1, int dim1, int x2, int dim2) {
	...
	void init_POOLMAXKernelFlat() {
		...
		final float _input[] = this.input.getDataFlat();
		final float _output[] = this.output.getDataFlat();
		final int ID = this.input.length_1();
		final int XD = this.indexes.length_1();
		final int INPUT_0_LENGTH = this.input.length_2();
		final int FILTER_INDEXES_LENGTH = this.indexes.length3();
		...
		kernel_pool = new Kernel() {
			...
			private static float getVI(float[] data, int x1, int dim1, int x2, int dim2) {
			...
			private static int getIndex(int x, int X, int y, int Y, int z, int Z) {
		    	int idx = X*(Y*z + y)+x;
		    	return idx;
		    }
			...
			static private int getAnchorScale(int x, int X, int y, int Y) {
				return X*Y;
			}
			static private int getAnchorDisp(int x, int X, int y, int Y) {
				return X*y+x;
			}
	...
	void init_POOLMAXKernel() {
		...
		final int ID = this.input.length_1();
		final int INPUT_0_LENGTH = this.input.length_2();
		final int FILTER_INDEXES_LENGTH = this.indexes.length3();//_indexes[0][0].length;
	...
	void init_RELUKernelFlat() {
		...
		kernel_perceptron = new Kernel() {
		    ...
		    static private  int getIndex(int x, int X, int y, int Y, int z, int Z) {
		    	int idx = X*(Y*z + y)+x;
		    	return idx;
		    }
			static private int getV(int[] data, int x1, int dim1, int x2, int dim2, int x3, int dim3) {
				return data[getIndex(x1, dim1, x2, dim2, x3, dim3)];
			}
			static private int getAnchored(int[] data, int anchor_scale, int anchor_displ, int ik) {
...
*/
    ...
    public void convoluteSoftMaxFlat() {
		...
		float aggregate = 0;
		for (int i = 0 ; i < output.length_1(); i ++) {
